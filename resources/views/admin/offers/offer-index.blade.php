            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="{{ route('offers.create') }}" class="btn btn-md btn-primary float-right">Add New Coupon</a>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @forelse($offers as $key => $value)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ ucfirst($value->name) }}</td>
                                                <td>{{ $value->courseDetails->course_name  ? $value->courseDetails->course_name : '' }}</td>
                                                <td>{{ $value->offer_percentage }}</td>
                                                <td>
                                                    @php
                                                        $from_date =  date_create($value->from_date);
                                                        $to_date =  date_create($value->to_date);
                                                    @endphp
                                                    {{ date_format($from_date, 'd-m-Y') }}</td>
